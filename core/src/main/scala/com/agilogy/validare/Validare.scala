...
trait Validare{
  ...
  private def toValidity(vf:(Context,ValidationFailure)):Validity.Invalid = Validity.Invalid(Map(vf._1 -> Seq(vf._2)))
  def valid[R](value: R): Valid[R] = Valid(value)
  def invalid[R](value:R, v0:ValidationFailure, vs:ValidationFailure *): Invalid[R] = {
    invalid(value, Context.Self -> v0, vs.map(Context.Self -> _) :_*)
  }
  def invalid[R](value:R, v0:(Context,ValidationFailure), vs:(Context,ValidationFailure) *): Invalid[R] = {
    Invalid(value, vs.map(toValidity).fold(toValidity(v0))(_ && _))
  }
  def illegal(v0:ValidationFailure, vs:ValidationFailure *): Illegal = {
    illegal(Context.Self -> v0, vs.map(Context.Self -> _) :_*)
  }
